...
#### HCI Cluster Registration ####

One of the first steps when deploying Azure Stack HCI is registration of the Cluster to your Azure Subscription. You can register the cluster in a number of ways including with Windows Admin Center and PowerShell.

Registering the cluster using Windows Admin Center is shown at the following URL:<BR>
https://learn.microsoft.com/en-gb/azure-stack/hci/guided-quick-deploy-eval?tabs=global-admin&tutorial-step=6

For PowerShell, for your convience a script has been added to automate the registration process. 

Run the below PowerShell script and run from the azure VM **AzSHCIHost001**:

`C:\AzHCI_Sandbox\AzSHCISandbox-main\Register-Cluster.ps1`

![Storage Spaces Direct enabled in the Create Cluster wizard](https://github.com/wcc-smiles/Azure-Stack-HCI-Workshop-Draft02/blob/main/Assets/2022-07-14%2007_27_48-AzSHCIHost001%20(1)%20-%20azshcihost001wcbdte.eastus.cloudapp.azure.com_3389%20-%20Remote%20.png)

You will be prompted for three additional items:

1. Login for the Contoso Domain Admin Account: **username:** Contoso\Administrator and **password:** Password01 
2. Provide a **devicelogin** via a browser, you will see this in yellow text with a code to input to "Microsoft.com/devicelogin".
3. Use your Azure portal credentials when requested.
4. From the **Azure Subcription** list select the relevant subscription:  **SubscriptionName**: **wccdemocompany1** and then Click **OK**.
5. From the **Azure Region** list select **eastus** and Click **OK**.
6. Ignore PowerShell error message **Get-Cluster : The cluster service is not running**.
7. Wait for PowerShell script to complete.

**Note: This will take approx 1-2 minutes**

8. Navigate back to the cluster from the **Admin Center VM** and will now show as **Registered** in **Windows Admin Center**.
